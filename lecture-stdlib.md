...
字符串（String）是一个使用频率非常高的引用类型，可以把它看作是一个由基本类型字符（char）构成的数组。在JVM内部，采用Unicode（UTF-16）作为字符集编码，对于非ISO-9959字符集的字符（比如中文的GB2312/GBK编码），开发者要注意不同编码之间的转换，否则就会出现一些没意义的乱码。
